...
diesel_high_status = diesel_all[diesel_all["status"] >= 2].copy()

d1, d2 = st.columns(2)
d1.metric(f"Diesel samples for current month ({report_month.strftime('%B %Y')})", len(diesel_current_month))
d2.metric("Diesel samples with status 2 or above", len(diesel_high_status))

st.markdown("### Diesel samples with status 2 or above")
if diesel_high_status.empty:
    st.info("No diesel samples with status 2 or above found.")
else:
    styled = diesel_high_status[
        [
            "customer",
            "sampno",
            "sampledate",
    ...
    st.dataframe(
        styled.style.apply(highlight_status, axis=1),
        use_container_width=True,
        hide_index=True,
    )
